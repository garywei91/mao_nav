...
export function withLankaAssistantCatalog(data) {
  const imported = assistantCategories()
  const importedNames = new Set(imported.map((category) => clean(category.name)))
  const supplemental = (data.categories || []).filter((category) => !importedNames.has(clean(category.name)))

  return {
    ...data,
    title: '兰卡68',
    categories: [...imported, ...supplemental],
  }
}
